...
/*indicator.onclick = function() {
  ...
  var colors = ["gray", "green", "red", "yellow"];
  this.style.color = colors[(colors.indexOf(color)+1)%4];
}*/
...
var postinginfo = document.querySelectorAll(".postinginfo")[0];
...
//var currentmd5 = MD5_hexhash(postingbody);
if (id && (id.length > 3) && (postinginfo.outerHTML.indexOf(id) > -1)) {
  ...
  if (postData) {
    ...
    if (currentText !== fullTexts[len-1]) { //the listing has changed
      ...
      //recentdiff = smartPatch_apply(verlist.reduce(function(prev, cur) {return prev + cur.patch;}, '')
    }
    else {
